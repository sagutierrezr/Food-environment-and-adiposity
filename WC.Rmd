...
```{r}
#=========================================================
# FOREST PLOTS - MODEL 4 (Food-related built environment x Waist Circumference)
# English labels | whisker end caps | darker/muted color palette
#=========================================================
...
# NOTE: internal codes (BOGOTA, MEDELLIN, ..., "Global") are kept
# unchanged so all matching logic below (is_pop, tibble(), etc.)
# still works. Only the DISPLAY label vector is translated
# ("Global" -> "Overall" for the figure axis).
# ---------------------------------------------------------
...
# because they must match the fitted model's actual factor levels
# (mod$coef names were generated from Spanish-coded data).
# English display text is supplied only via `labels =` in
# scale_color_manual(), so the legend shows English without
# breaking the coefficient look-up.
# ---------------------------------------------------------
...
sym_x_scale <- function(lwr, upr, target_breaks = 7) {
  ...
  list(limits = c(-max_break, max_break), breaks = breaks)
}

#=========================================================
# FOREST PLOT 1: Supermarket density (DENS_HEALTH_LOG_ZC), by city
# File: Fig9A_supermarket_density_by_city.png
# NOTE: this represents the reference education stratum (Elementary
# or lower), since the model includes a density x education
# interaction. Make sure the caption states this explicitly.
#=========================================================
...
p_health_city <- ggplot(plot_df, aes(x = slope, y = city_y, color = CITY_NAME)) +
  ...
  # height > 0 adds a short vertical cap at each end of the CI, as in the example figure
...
ggsave("Fig9A_supermarket_density_by_city.png", p_health_city,
       width = export_w, height = export_h, units = "in",
       dpi = export_dpi, bg = "white")

#=========================================================
# FOREST PLOT 2: Supermarket density x Educational level, by city
# File: Fig9B_supermarket_density_by_education.png
#=========================================================
...
# Fixed scale: -12 to 12 (kept identical to the original spec)
x_limits <- c(-12, 12)
x_breaks <- seq(-12, 12, by = 3)

p_health_edu <- ggplot(plot_df, aes(x = slope, y = y, color = EDU_GROUP)) +
  annotate("rect", xmin = -Inf, xmax = Inf,
           ymin = y_prom - 0.42, ymax = y_prom + 0.42,
           fill = "grey92", alpha = 0.6) +
  geom_vline(xintercept = setdiff(x_breaks, 0),
             linetype = "dotted", linewidth = 0.4, color = "grey70") +
  geom_vline(xintercept = 0, linetype = "dashed",
             linewidth = 0.7, color = "grey35") +
  # smaller cap height here: three sub-rows per city are only 0.24 apart
...
ggsave("Fig9B_supermarket_density_by_education.png", p_health_edu,
       width = export_w, height = export_h, units = "in",
       dpi = export_dpi, bg = "white")

#=========================================================
# FOREST PLOT 3: Restaurant density (DENS_UNHEALTH_LOG_ZC), by city
# File: Fig9C_restaurant_density_by_city.png
# NOTE: same caveat as Plot 1 - reference education stratum only.
#=========================================================
...
ggsave("Fig9C_restaurant_density_by_city.png", p_unhealth_city,
       width = export_w, height = export_h, units = "in",
       dpi = export_dpi, bg = "white")

#=========================================================
# FOREST PLOT 4: Restaurant density x Educational level, by city
# File: Fig9D_restaurant_density_by_education.png
#=========================================================
...
ggsave("Fig9D_restaurant_density_by_education.png", p_unhealth_edu,
       width = export_w, height = export_h, units = "in",
       dpi = export_dpi, bg = "white")
```
